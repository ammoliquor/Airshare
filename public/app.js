...
// Render files list into DOM (instant 0ms operation)
...
// Load and render file lists from server
...
dropZone.addEventListener('click', (e) => {
  if (e.target && e.target.closest('#mobile-paste-file-btn')) return;
  fileInput.click();
});
...
// Helper to show visual status on the clipboard card
function showClipboardStatus(msg, type = 'success') {
  if (!clipboardStatus) return;
  clipboardStatus.textContent = msg;
  clipboardStatus.className = 'clipboard-status active ' + (type === 'error' ? 'error' : '');
  setTimeout(() => {
    clipboardStatus.classList.remove('active');
  }, 2000);
}

// Send clipboard text to backend & sync across devices
async function sendClipboardText(text) {
  try {
    ...
    if (res.ok) {
      showClipboardStatus('Synced', 'success');
    } else {
      showClipboardStatus('Sync failed', 'error');
    }
  } catch (err) {
    console.error('Error sending clipboard:', err);
    showClipboardStatus('Network error', 'error');
  }
}

// Clipboard paste reader
async function handlePasteFromClipboard() {
  showClipboardStatus('Reading clipboard...', 'info');
  ...
  if (navigator.clipboard && navigator.clipboard.readText) {
    try {
      ...
      if (text) {
        ...
        showClipboardStatus('Pasted & Synced', 'success');
        return true;
      }
    } catch (err) {
  ...
  // Fallback: focus textarea for native paste popup
  clipboardTextarea.focus();
  showClipboardStatus('Tap Paste in popup', 'info');
  return false;
}
...
if (sendClipboardBtn) {
  sendClipboardBtn.addEventListener('click', () => {
    sendClipboardText(clipboardTextarea.value);
  });
}
...
// Auto-sync when pasting text directly into textarea
clipboardTextarea.addEventListener('paste', () => {
  setTimeout(() => {
...
async function copyToClipboard(text) {
  // Always try modern asynchronous Clipboard API first (supported in iOS Safari 13.4+, Android Chrome, and PC)
  ...
  // Bulletproof fallback logic for older legacy browsers
...
// Instant synchronous hydration from server-injected state (0ms first-paint rendering)
if (window.__INITIAL_DATA__) {
  const initData = window.__INITIAL_DATA__;
  if (initData.clipboard && clipboardTextarea) {
    clipboardTextarea.value = initData.clipboard;
  }
  if (initData.files) {
    renderFilesList(initData.files);
  }
  if (initData.downloadDir) {
    updateFolderUI(initData);
  }
}

// Main startup (executes immediately to eliminate initial network delay)
function startup() {
  initWebSocket();
  // If not already hydrated from server-side __INITIAL_DATA__, load via network
  if (!window.__INITIAL_DATA__) {
    loadFilesList();
    loadFolderConfig();
  }
